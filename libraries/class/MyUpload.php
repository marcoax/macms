...
class MyUpload{
  ...
  var $cls_max_filesize = 10000000; // Max file size.
  var $cls_filesize ="";            // Actual file size.
  var $cls_arr_ext_accepted = array(
      ".doc", ".xls", ".txt", ".pdf", ".gif", ".jpg", ".zip", ".rar", ".ppt",
      ".html", ".xml", ".tiff", ".jpeg", ".png" ,".htm",".gz",".csv",".doc");
...
*/
class MyUploadPath extends MyUpload {

 function MyUploadPath($site,$dir='',$lang='',$path='../../'){
    $this->site=$site;
    $this->lang=$lang;
    $this->dir=$dir;
    $this->Brand=$dir;
 ...
 function getPath(){
      ...
      $this->dir=($this->tipo=='myImg')?"img":$this->dir."doc";
      $this->url=$this->path.$this->site."/repository/".$this->dir."/";
      if($this->tipo=='myPdf')$this->url=$root."/pdf_main/pdf/". $this->Brand."/";
      return $this->url;
 }

 function makeUpload($tab,$field='filename'){
      $this->getPath();
      $number_of_files=count($_FILES[$this->tipo]['name']);
      for($i=0;$i<$number_of_files;$i++){
        $this->MyUpload($_FILES[$this->tipo]['name'][$i], $_FILES[ $this->tipo]['tmp_name'][$i], $_FILES[ $this->tipo]['size'][$i],"thisname" );
         $result=$this->upload($this->url);
          if($result){
          if($this->multi)$fileName=$tab.'_'.$field.':'.($i+1);
          else  $fileName=$tab.'_'.$field;
             if($_FILES[$this->tipo]['name'][$i])$_POST[$fileName]=$_FILES[$this->tipo]['name'][$i];
          }

         // echo    date ("d-m-Y", filemtime($this->url."/".$_FILES[$this->tipo]['name'][$i]));
      }
  }
}
?>