	 ...
	 function getData($data,$type,$nullCol=''){
		...
		for($i=0;$i<count($data);$i++){
		  ...
		  if($this->Multi){
		   ...
		   $valore="";
		   if($i==0 && empty($this->modeFull)) {
		   	        $this->where.=$data[0]."='".$curField."'";
		   			if($this->seqOracle) {
						$this->campi.=$data[$i].',';
						$this->valori.=$this->seqOracle.'.nextVal'.',';
					}
		   }
		   else {
			       
		   	       //echo $type[$i].'<br> ';
		   	       //  se  � un  campo TIMESTAMP  in  oracle miauscolo
		   	       
		           if($type[$i]=="TIMESTAMP"){$valore='SYSDATE';}
				   ...
				   if(!empty($curField)){
                    // se  il  contuto  � un  array  faccio implode di tipo  set
                       ...
                       elseif($this->isOracle)$valore=str_replace("'", "''",$curField);
                       else $valore=addslashes(trim($curField));
					   //  se  � un  campo DATE  in  oracle
				       if($type[$i]=="DATE"){
					        $this->isDate($valore);
                 ...
                 else if($type[$i]=="TIMESTAMP" || $type[$i]=="DATE"){
...
class subInsert extends Action {
  ...
  function getQuery($mode,$tabella){
   ...
   //lista  dati, separatore e   per oracle  mette il nome del campo in maiuscolo, tabella  corrente  per   evitare sovvrapposizioni di  dat
  function setCurArray($curArr,$sep,$uppercase='',$mainTab=''){
     $curArray= array();
	 while (list($label,$value) = each($curArr)){
     $curItem=split($sep,$label);
	 $curEle=$curItem[1];
	 if(String::ma_not_null($curItem[2]))$curEle.="_".$curItem[2];
	 $curEle=($uppercase==1)?strtoupper($curEle):$curEle;
