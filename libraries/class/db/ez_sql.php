<?php
	...
	define("EZSQL_DB_USER", DB_SERVER_USERNAME);			// <-- mysql db user
	define("EZSQL_DB_PASSWORD", DB_SERVER_PASSWORD);		// <-- mysql db password
	define("EZSQL_DB_NAME", DB_DATABASE);		// <-- mysql db pname
	...
	class db {
		...
		function debug($showResult=0)
		{
			...
			if ( $this->last_result && $showResult=1)
			{
			...
			echo "</blockquote>".$this->donation()."<hr noshade color=dddddd size=1>";
