	...
	class ezSQL_oracle8_9 extends ezSQLcore
	{
		...
		*/
		...
		function ezSQL_oracle8_9($dbuser='', $dbpassword='', $dbname='')
		{
			...
			{
				$this->connect($dbuser, $dbpassword, $dbname);
			}
		}
		...
		*/
		...
		function query($query)
		{
			...
			// modified  by  dba+ webmaster il 05/05/2010
			$stmt = OCIParse($this->dbh, $query);
			...
			if(is_array($this->arrayBind) && count($this->arrayBind)) {
				OCIBindbyname($stmt, ':a', $this->arrayBind['a']);
				OCIBindbyname($stmt, ':b', $this->arrayBind['b']);
				$i=1;
				foreach ($this->arrayBind as $array_title => $array_value){
               // ':'.$array_title.'-'.$array_value.'<br>';
			    //echo $bindVar=':'.$array_title.'<br>';
	 			//OCIBindbyname($stmt, ':a', $array_value);
	            $i++;
				}
			...
			else
			{
				...
				if ( $num_cols = @OCINumCols($stmt) )
				{
	    			...
	    			for ( $i = 1; $i <= $num_cols; $i++ )
	    			{
	    				...
	    				$this->col_info[($i-1)]->not_null = @OCIColumnIsnull($stmt,$i);
		...
		/*ma  12-2008*/
		//log Off the oracle Connection
		function close_conn(){
		  OCILogoff($this->dbh);
		}
		
		/**********************  this  function  check  if a  table   is  present in  the  curren  db schema*/
		/*ma  12-2009*/
		function  ckTable($table){
